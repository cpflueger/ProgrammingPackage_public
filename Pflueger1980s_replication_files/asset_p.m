...
classdef asset_p
    ...
    properties
        ...
        Irf1, Irf2, Irf3, Irf4,    %   impulse responses in response to first, second, third and fourth structural shock
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % SolveAll is the main method that calls all other methods in asset_p
        % It first create grids, then solves for risk-neutral asset prices
        % (if this option is enabled) and the solves and simulates asset
        % prices with risk premia
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function asset = SolveAll(assetInput,macro_dyn, num_set)
            % Initialize output asset_p class
            rng(0)
            if isempty(assetInput.initialShockVec) 
                assetInput.initialShockVec=[0,0,0.0465,0];
            end
            ...
            %% Evaluate lambda(shat) at all gridpoints
            ...
            % preliminary value for log-linearization constant rho
            asset.rho = 0.99;
            %% Set temporary numerical values for simulation
            num_set.T = 10000;
%            num_set.Nsim = 2;
            ...
            if assetInput.risk_neutral_run == 1
                ...
                %log-linearization constant to decompose risk-neutral returns into cash flow news and discount rate news
            ...
            %% Solve for Price-Dividend Ratio, real and nominal bond prices with risk premia
            tic
            asset = asset.computeFn21(num_set,macro_dyn);
            toc
            %if not solving for risk-neutral asset prices set placeholder
            %risk-neutral=regular asset prices
            if assetInput.risk_neutral_run == 0
                asset.G_rn = asset.G;
                asset.Bn_rn = asset.Bn;
                asset.Bnom_rn = asset.Bnom;
            end
            
            %% Calculate the various asset pricing moments needed through simulations
            disp('Simulate moments')
            asset.risk_neutral_run = 1;
            tic
            asset = asset.SimulateMoments(num_set, macro_dyn );
            toc
            %%get asset pricing responses to shocks
            num_set.Nsim=500;
        end
...
%            Nbonds  = num_set.Nbonds;
            Nbins   = num_set.Nbins;
            mat     = min(num_set.mat, num_set.Nbonds);
            h       = num_set.h;
%            gamma     = macro_dyn.gamma;
            ...
            %XXXFL: leverage of beta-sorted portfolios.
...
%            betaq=macro_dyn.betaq;
            %sigma_vec = macro_dyn.sigma_vec;
            %Macroeconomic equilibrium - may be different from equilibrium
            %used for pricing assets
            if isempty(macro_dyn.Q_out)==0
                Q         = macro_dyn.Q_out;
            end
            if isempty(macro_dyn.P_out)==0
            P         = macro_dyn.P_out;
            end
            sigma_vec = macro_dyn.sigma_vec_out;
            sigmac = sqrt([1,0,0]*Q*diag(sigma_vec)*([1,0,0]*Q)');
%            QM        =[1,0,0]*Q;
...
%             const3 = ([0,0,1]-[0,1,0]* P)* Ainv;
            ...
            Pnomplus5dim       = log(reshape(asset_pI.Bnom(mat-1,:,:,:), N ,N ,N, N2, sizexm));
            Pnomminus5dim      = log(reshape(asset_pI.Bnom(mat-2,:,:,:), N ,N ,N, N2, sizexm));
            ...
            Pplus5dim          = log(reshape(asset_pI.Bn(mat-1,:,:,:), N, N, N, N2, sizexm));
            Pminus5dim         = log(reshape(asset_pI.Bn(mat-2,:,:,:), N, N, N, N2, sizexm));
            ...
            if asset.risk_neutral_run == 1
                ...
                Pnomplus5dim_rn     = log(reshape(asset_pI.Bnom_rn(mat-1,:,:,:), N ,N ,N, N2, sizexm));
                Pnomminus5dim_rn    = log(reshape(asset_pI.Bnom_rn(mat-2,:,:,:), N ,N ,N, N2, sizexm));
                ...
                Pplus5dim_rn        = log(reshape(asset_pI.Bn_rn(mat-1,:,:,:), N, N, N, N2, sizexm));
                Pminus5dim_rn       = log(reshape(asset_pI.Bn_rn(mat-2,:,:,:), N, N, N, N2, sizexm));
                P5y5dim_rn          = log(reshape(asset_pI.Bn_rn(19,:,:,:), N ,N ,N, N2, sizexm));
            else
                
                G5dim_rn             = log(reshape(asset_pI.G, N, N, N,N2,sizexm));
                
                Pnomplus5dim_rn      = log(reshape(asset_pI.Bnom(mat-1,:,:,:), N ,N ,N, N2, sizexm));
                Pnomminus5dim_rn     = log(reshape(asset_pI.Bnom(mat-2,:,:,:), N ,N ,N, N2, sizexm));
                Pnomhq5dim_rn        = log(reshape(asset_pI.Bnom(h-1,:,:,:), N ,N ,N, N2, sizexm));
                Pnom5y5dim_rn        = log(reshape(asset_pI.Bnom(19,:,:,:), N ,N ,N, N2, sizexm));
                
                Pplus5dim_rn         = log(reshape(asset_pI.Bn(mat-1,:,:,:), N, N, N, N2, sizexm));
                Pminus5dim_rn        = log(reshape(asset_pI.Bn(mat-2,:,:,:), N, N, N, N2, sizexm));
 ...
 %                   ztildesim(:,t) = Ptilde*ztildesim(:,t-1)+eps(:,t);
                    ...
                    ztildesim(:,t) = A*Yhat(:,t);
                    % Dynamics for surplus consumption ratio relative to steady-state
                    shatsim(t)     = ...
                        theta0*shatsim(t-1)+theta1*const4*ztildesim(:,t-1)+theta2*const4*ztildesim(:,t-2)+...
                    ...
                    xminusinterp   = max(xminuslower, min(const4*ztildesim(:,t-1),xminusupper)); % this is the trucated last period output gap
                    csim(t)        = g+csim(t-1)+(Yhat(1,t)-phi*Yhat(1,t-1));
                    piast(t)       = piast(t-1)+uast(t);
                    
                    % scaling factors for nominal bonds
                    ePplus    = exp(mat*piast(t));
                    ePminus   = exp((mat-1)*piast(t));
                    eP5y      = exp(20*piast(t));
                    ePhq      = exp(h*piast(t));
...
%                    ztildesim_pre   = Ptilde*ztildesim(:,t-1)+eps_pre(:,t);
                    ...
                    if asset.FOMC_run ==1
                        ...
                        ePplus_pre  = exp(mat*piast_pre(t));
                ...
                y10nom              = -400*log(Pnomplus)/mat;
                y10real             = -400*log(Pplus)/mat;
                y10nom_rn           = -400*log(Pnomplus_rn)/mat;
                y10real_rn          = -400*log(Pplus_rn)/mat;
                ...
                y10nom_pre              = -400*log(Pnomplus_pre)/mat;
                y10nom_FOMC             = y10nom-y10nom_pre;
                
                y10real_pre             = -400*log(Pplus_pre)/mat;
                ...
                y10nom_rn_pre           = -400*log(Pnomplus_rn_pre)/mat;
                y10nom_rn_FOMC          = y10nom_rn - y10nom_rn_pre;
                
                y10real_rn_pre          = -400*log(Pplus_rn_pre)/mat;
                ...
                %XXXFL: risk-neutral beta-sorted portfolio returns,
                %matrix dimension T-by-#port.
                delta_temp = repmat(delta_betaport', T, 1);
                ...
                dDelta_bar = (1/8)*(dDelta(1:end-7)+dDelta(2:end-6)+dDelta(3:end-5)+dDelta(4:end-4)+dDelta(5:end-3)+dDelta(6:end-2)+dDelta(7:end-1)+dDelta(8:end));
                dDelta_bar = 0.5*(dDelta_bar(1:end-8)+dDelta_bar(9:end));
                dDelta_bar = 0.5*(dDelta_bar(1:end-16)+dDelta_bar(17:end));
                dDelta_bar = 0.5*(dDelta_bar(1:end-32)+dDelta_bar(33:end));
                ...
                piast              = piast(burn-1:end)';
                y10nom             = y10nom(burn-1:end);
                y10real            = y10real(burn-1:end);
                y10nom_rn          = y10nom_rn(burn-1:end);
                y10real_rn         = y10real_rn(burn-1:end);
                y5real             = y5real(burn-1:end);
                y5real_rn          = y5real_rn(burn-1:end);
                y5nom              = y5nom(burn-1:end);
                y5nom_rn           = y5nom_rn(burn-1:end);
                yhqnom             = yhqnom(burn-1:end);
                yhqnom_rn          = yhqnom_rn(burn-1:end);
                rfr                = rfr(burn-1:end);
                if burn>200
                    dDelta_bar         = dDelta_bar((burn-129):end); %align dates
                else
                    dDelta_bar         = dDelta_bar((burn-65):end); %align dates
                end
                eps                = eps(:,burn-1:end);
%                approxErrI         = approxErrI(burn-1:end);
                ...
                reteq              = real(log((1/delta)*Ret - ((1-delta)/delta)*exp(rfr(1:end-1)'/400)));
                reteq_rn           = real(log((1/delta)*Ret_rn - ((1-delta)/delta)*exp(rfr(1:end-1)'/400)));
                ...
                ret_betaport = log((1./delta_temp).*Ret_temp - ((1-delta_temp)./delta_temp).*exp(rfr_temp)); %XXXFL
                ...
                %changes in nominal and real yields               
                Delta_y10nom=y10nom(5:end)-y10nom(1:end-4);
                Delta_y10nom_rn=y10nom_rn(5:end)-y10nom_rn(1:end-4);
                Delta_y10real=y10real(5:end)-y10real(1:end-4);
                Delta_y10real_rn=y10real_rn(5:end)-y10real_rn(1:end-4);

%%
                % Drop observations to ensure that returns and prices have the same length
                PDlev              = PDlev(3:end);
                reteq              = reteq(3:end);
                reteq_rn           = reteq_rn(3:end);
                ret_betaport       = ret_betaport(3:end,:); 
                ret_rn_betaport    = ret_betaport(3:end,:);
                retnom             = retnom(3:end);
                retnom_rn          = retnom_rn(3:end);
                retreal            = retreal(3:end);
                retreal_rn         = retreal_rn(3:end);
                breakeven          = breakeven(3:end);
                Yhat               = Yhat(:,3:end);
                shatsim            = shatsim(3:end);
                csim               = csim(3:end);
                piast              = piast(3:end);
                y10nom             = y10nom(3:end);
...
%% Regress FOMC asset returns onto nominal rate changes on FOMC dates
               ...
               nomyieldsMP = [nomInfoEffectTemp1(2),...
                ...
                BondPremium2    = 4*mean(retnom) +mean(rfr_nom)-mean(yhqnom);
                % Std of bond returns
                Stdnom          = std(retnom)*2;
                Stdnom_rn       = std(retnom_rn)*2;
                ...
                %risk-neutral correlations and betas
                bondstock_corr_temp     = corrcoef(retnom_rn, reteq_rn);
                tipsstock_corr_temp     = corrcoef(retreal_rn, reteq_rn);
                correlations_rn       = [bondstock_corr_temp(1,2), tipsstock_corr_temp(1,2)];
                
                % Risk neutral betas
                beta_temp           = regress(retnom_rn, [ones(T-burn1+1,1), reteq]);
                beta_nom_rn         = beta_temp(2);
                ...
                %risk-neutral beta
                beta_temp          = regress(retreal_rn, [ones(T-burn1+1,1), reteq]);
                beta_real_rn       = beta_temp(2);
                
                cov1=cov(reteq, reteq);
                cov2=cov(reteq_rn, reteq);
                cov3=cov(reteq, reteq_rn);
                cov4=cov(reteq_rn, reteq_rn);
                
                var_stocks_decomp=[cov4(1,2), cov3(1,2); cov2(1,2), cov1(1,2)];
                
                cov1=cov(retnom, retnom);
                cov2=cov(retnom_rn, retnom);
                cov3=cov(retnom, retnom_rn);
                cov4=cov(retnom_rn, retnom_rn);
                
                var_nom_decomp=[cov4(1,2), cov3(1,2); cov2(1,2), cov1(1,2)];
                
                cov1=cov(retreal, retreal);
                cov2=cov(retreal_rn, retreal);
                cov3=cov(retreal, retreal_rn);
                cov4=cov(retreal_rn, retreal_rn);
                
                var_real_decomp=[cov4(1,2), cov3(1,2); cov2(1,2), cov1(1,2)];
                
                cov1=cov(retnom, reteq);
                cov2=cov(retnom_rn, reteq);
                cov3=cov(retnom, reteq_rn);
                cov4=cov(retnom_rn, reteq_rn);
                
                cov_nom_decomp=[cov4(1,2), cov3(1,2); cov2(1,2), cov1(1,2)];
                
                cov1=cov(retreal, reteq);
                cov2=cov(retreal_rn, reteq);
                cov3=cov(retreal, reteq_rn);
                cov4=cov(retreal_rn, reteq_rn);
                
                cov_real_decomp=[cov4(1,2), cov3(1,2); cov2(1,2), cov1(1,2)];
                
                %regress stock and bond returns onto fundamental shocks
                stats_nom=regstats(retnom, (inv(Q(1:3,1:3))*Ainv*eps)');
                stats_real=regstats(retreal, (inv(Q(1:3,1:3))*Ainv*eps)');
                stats_eq=regstats(reteq, (inv(Q(1:3,1:3))*Ainv*eps)');
                stats_nom_rn=regstats(retnom_rn, (inv(Q(1:3,1:3))*Ainv*eps)');
                stats_real_rn=regstats(retreal_rn, (inv(Q(1:3,1:3))*Ainv*eps)');         
                
                shock_decomp_nom=stats_eq.beta(2:4)'.*std((inv(Q(1:3,1:3))*Ainv*eps)').^2.*stats_nom.beta(2:4)';
                shock_decomp_nom_rn=stats_eq.beta(2:4)'.*std((inv(Q(1:3,1:3))*Ainv*eps)').^2.*stats_nom_rn.beta(2:4)';             
                
                shock_decomp_real=stats_eq.beta(2:4)'.*std((inv(Q(1:3,1:3))*Ainv*eps)').^2.*stats_real.beta(2:4)';
                shock_decomp_real_rn=stats_eq.beta(2:4)'.*std((inv(Q(1:3,1:3))*Ainv*eps)').^2.*stats_real_rn.beta(2:4)';
                ...
                %XXXFL: betas of beta-sorted portfolios.
                ...
                piChanges_ma= movmean(piChanges , 40 );
                Epi10Changes_Vol = std((1-macro_dyn.zeta)*(Delta_y10nom_rn(4:end)-Delta_y10real_rn(4:end))+macro_dyn.zeta*piChanges_ma(1:end-1)');
                Epi10Changes_Vol_rational = std((Delta_y10nom_rn(4:end)-Delta_y10real_rn(4:end)));
...
%            asset.additional_moments     = [mean(fracmax), stdApproxErrI];
            ...
            nominalBonds.expReturn                 = BondPremium2;
            nominalBonds.vol                       = mean(Stdnom);
            nominalBonds.vol_rn                    = mean(Stdnom_rn);
            ...
            nominalBonds.betaNom_scaled            = -100*beta_nom*(4/mat);
            ...
            realBonds.vol_rn               = mean(Stdreal_rn);
            ...
            realBonds.betaRealStock_scaled = -100*beta_real*(4/mat);
            ...
            crossAsset.betaNom_rn       = beta_nom_rn;
            crossAsset.betaReal         = beta_real;
            crossAsset.betaReal_rn      = beta_real_rn;
            ...
            crossAsset.var_stocks       = var_stocks_decomp;
            crossAsset.var_nom          = var_nom_decomp;
            crossAsset.var_real         = var_real_decomp;
            crossAsset.cov_nom          = cov_nom_decomp;
            crossAsset.cov_real         = cov_real_decomp;
            crossAsset.shock_decomp_nom = shock_decomp_nom;
            crossAsset.shock_decomp_nom_rn = shock_decomp_nom_rn;
            crossAsset.shock_decomp_real = shock_decomp_real;
            crossAsset.shock_decomp_real_rn = shock_decomp_real_rn;
            ...
            macroDynamics.Epi10Changes_Vol = Epi10Changes_Vol;
            macroDynamics.Epi10Changes_Vol_rational = Epi10Changes_Vol_rational;
            macroDynamics.y10nom_Vol    = std(Delta_y10nom);
            macroDynamics.y10real_Vol    = std(Delta_y10real);
            ...
            simulated.PD                  = PDlev(3:end);
            simulated.x                   = 100*Yhat(1,:)';
        ...
        function asset = computeFn21(asset_pI, num_set, macro_dyn)
            ...
            Q = macro_dyn.Q;
            Ainv = macro_dyn.Ainv;
            A = macro_dyn.A; % LY 11/27/2024: Set matrix A to compute bond prices
            Sigmau = macro_dyn.Sigmau;
            Ptilde = macro_dyn.Ptilde;
            g = macro_dyn.g;
            phi = macro_dyn.phi;
            sigmac = macro_dyn.sigmac;
            vast = macro_dyn.vast;
            vecp = macro_dyn.vecp; % LY 11/27/2024: Set vector vecp to compute bond prices
            rf = macro_dyn.rf;
            QM = macro_dyn.QM;
            P = macro_dyn.P;
            sigmaperp = macro_dyn.sigmaperp;
            sigmapperp = macro_dyn.sigmapperp; % LY 11/27/2024: Set scalar sigmapperp to compute bond prices
            delta = macro_dyn.delta;
            theta1=macro_dyn.theta1;
            theta2=macro_dyn.theta2;
            ...
            if risk_neutral_run == 0
                ...
                const1 =  g;
                const2 =  tau*[1,0,0]*( P- phi*eye(3))* Ainv;
                const3 = ([0,0,1]-[0,1,0]* P)* Ainv;
                %matrix pre-multiplying state variable for consumption claims
                const1c=[1,0,0]*((gamma*theta1-gamma*phi+phi)*eye(3)+(gamma-1)*P)* Ainv;
                %log discount rate
                const2c=log(macro_dyn.betaq)-(gamma-1)*g;
                ...
                for i1=1: N1
                    ...
                    for i2=1: N2
                        ...
                        for i3=1: sizexm
                            ...
                            fnew(i1,i2,i3)= const2c -const1c*Z(i1,:)'-gamma*theta2*xmgrid(i3)  - gamma*(theta0-1)* shat(i2) + 0.5*(gamma* lambdas(i2)+ (gamma- 1))^2*sigmac^2;
                        end
                    end
                end
                ...
                for i1=1: N1
                    ...
                    for i2=1: N2
                        % i3 loops over grid points for x_{t-1}
                        for i3=1: sizexm
                            %fnew(i1,i2,i3)= const1 +rho_a*xmgrid(i3)+ const2*Z(i1,:)' - rf - const3*Z(i1,:)' + 0.5*sigmac^2;
                            fnew(i1,i2,i3)= const2c - const1c*Z(i1,:)' - gamma*theta2*xmgrid(i3) + 0.5*sigmac^2;
                        end
                    end
                end
            end
            ...
            %define temporary constants outside the loop to calculate them only once
            const1 = -([0,0,1]+(1-phi)*[1,0,0]-[0,1,0]* P)*(eye(3)+ P)* Ainv;
            const2 = -[0,0,1]*(eye(3)+ P)* Ainv;
            vr2     = ([0,0,1]+(1-phi)*[1,0,0]-[0,1,0]* P)* Q;
            vbn2    = [0,1,1]*Q+vecp'*A*Q+2*[0,0,0,1]; % LY 11/27/2024: I updated as in the Appendix Equation (A155), I added the term "vecp'*A*Q"
            %asset prices with risk premia
            if risk_neutral_run == 0
                %i1 loops over grid points for \tilde Z
                for i1 = 1:N1
                    ...
                    for i2 = 1:N2
                        ...
                        for i3=1: sizexm
                            ...
                            bnnew(i1,i2,i3) = const2* Z(i1,:)'+0.5*vbn2* Sigmau*vbn2'+0.5*sigmapperp^2+gamma*(1+lambdas(i2))*QM*Sigmau*vbn2'-2* rf; % LY 11/27/2024: I updated the two-period nominal bond price as in the Appendix Equation (A154). I added the term "0.5*sigmapperp^2" 
                        end
                    end
                end
                ...
                for i1 = 1:N1
                    ...
                    for i2 = 1:N2
                        ...
                        for i3 = 1:sizexm
                            ...
                            bnnew(i1,i2,i3) = const2* Z(i1,:)' + 0.5 * vbn2* Sigmau * vbn2' + 0.5*sigmapperp^2 - 2 * rf; % LY 12/05/2024: I updated this according to the new risk-neutral solution, I added the term "0.5*sigmapperp^2" 
                        end
                    end
                end
            end
            ...
            vpi          = [0,1,0]* Ainv+vecp'; % LY 11/27/2024: I updated as in the Appendix Equation (A153), I added the term "vecp'"
            vpi1         = vpi(1)* xGL';
            ...
            %const1 =  tau*[1,0,0]*( P- phi*eye(3))* Ainv;
            %const2 =  g;
            const3 = ([0,0,1]+(1-phi)*[1,0,0]-[0,1,0]* P)* Ainv;
                        ...
                        for i3=1: sizexm
                            ...
                            for i4=1: N3
                                ...
                                fs_plus          = (1-T2').*FS1+T2'.*FS2;
                                    ...
                                    if risk_neutral_run == 0
                                        %fs_plus = fs_plus' + const2 + rho_a*xmgrid(i3) + const1 * Z(i1,:)' - rf - const3 * Z(i1,:)' + const4 * (1 - 2* shat(i2)) - (gamma * (1+ lambdas(i2))- 1) * ve1;
                                        fs_plus = fs_plus' + const2c  - const1c*Z(i1,:)' - gamma * theta2 * xmgrid(i3)- gamma*(theta0-1)* shat(i2)  - (gamma * (1+ lambdas(i2))- 1) * ve1;
                                    else
                                        %fs_plus = fs_plus' + const2 + rho_a*xmgrid(i3) + const1 * Z(i1,:)' - rf - const3 * Z(i1,:)' + ve1;
                                        fs_plus = fs_plus' + const3c -  const1c * Z(i1,:)' - gamma * theta2 * xmgrid(i3) + ve1;
                                    end
                                ...
                                if n < Nbonds
                                    ...
                                    if risk_neutral_run == 0
                                        ...
                                        bns_plus = bns_plus' - rf + 0.5*sigmapperp^2 - [0,0,1] * Ainv * Z(i1,:)' - 0.5 * gamma * (1- theta0) * (1 - 2*shat(i2)) - ...
                                            gamma * (1+ lambdas(i2)) * ve1 - vpi1 - (n+1) * uast1 + 0.5 * (n+1)^2 * sigmaperp^2; % LY 11/27/2024: I updated as in the Appendix Equation (A153), I added the term "0.5*sigmapperp^2"
                                    else
                                        bs_plus = bs_plus' - rf - const3 * Z(i1,:)';
                                        bns_plus = bns_plus' - rf + 0.5*sigmapperp^2 - [0,0,1] * Ainv * Z(i1,:)' - vpi1 - (n+1) * uast1 + 0.5 * (n+1)^2 * sigmaperp^2; % LY 12/05/2024: I updated this according to the new risk-neutral solution, I added the term "0.5*sigmapperp^2" 
                                    end
        ...
        function asset = simulateStructuralIRF(asset_pI, macro_dyn, num_set)
            ...
            while j<=Nsim
                ...
                u(:, burn+2)   = u(:, burn+2)+(asset.initialShockVec)';
                ...
                Pminus_rn      = ZT1;
                csim           = ZT1;
                csim_ss = ZT1; %XXXFL: consumption under constant growth rate g.
                piast          = ZT1;
                ...
                csim               = csim(burn-1:end);
                csim_ss = csim_ss(burn-1:end); %XXXFL
                ...
                iIrf       = Y(3,1:20) - Y(3,1);
                PDIrf      = 100*(log(PDlev(1:20)) - log(PDlev(1)));
                ...
                PDIrf_rn       = 100*(log(PDlev_rn(1:20)) - log(PDlev_rn(1)));
            ...
            Irftemp.x=xIrf;
            Irftemp.c = cIrf * 100; %XXXFL: IRF of consumption relative to constant growth.
            Irftemp.pi=piIrf;
            Irftemp.i=iIrf;
            Irftemp.PD=PDIrf;
